...
-- ================== COMPETITIVE PROGRAMMING (C++ ONLY) ==================
-- C++ Template
...
map("n", "<F5>", function()
    vim.notify("Running C++ tests...", vim.log.levels.INFO)
    vim.cmd("w")
    vim.cmd("CompetiTest run")
end, {desc = "Run C++ tests"})

-- F6: Quick compile and run
map("n", "<F6>", function()
    vim.cmd("w")
    ...
    vim.cmd("split | terminal g++ -std=c++17 -O2 -Wall -Wextra -o " .. name ..
                " " .. file .. " && ./" .. name)
    vim.cmd("resize 15")
    vim.cmd("startinsert")
end, {desc = "Compile and run C++"})

-- F8: Run with input.txt
map("n", "<F8>", function()
    vim.cmd("w")
    local file = vim.fn.expand("%")
    local name = vim.fn.expand("%:r")
    ...
    if vim.fn.filereadable("input.txt") == 1 then
        vim.notify("Running with input.txt...", vim.log.levels.INFO)
        vim.cmd(
            "split | terminal g++ -std=c++17 -O2 -o " .. name .. " " .. file ..
                " && ./" .. name .. " < input.txt")
    else
        vim.notify("input.txt not found! Running normally...",
                   vim.log.levels.WARN)
        vim.cmd(
            "split | terminal g++ -std=c++17 -O2 -o " .. name .. " " .. file ..
                " && ./" .. name)
...
-- ================== WEB DEVELOPMENT ==================
map("n", "<leader>ti", function() vim.cmd("TSToolsOrganizeImports") end,
    {desc = "Organize imports"})
map("n", "<leader>ts", function() vim.cmd("TSToolsSortImports") end,
    {desc = "Sort imports"})
map("n", "<leader>tu", function() vim.cmd("TSToolsRemoveUnused") end,
    {desc = "Remove unused imports"})
map("n", "<leader>tf", function() vim.cmd("TSToolsFixAll") end,
    {desc = "Fix all"})
map("n", "<leader>ta", function() vim.cmd("TSToolsAddMissingImports") end,
...
map("n", "<leader>nc", "<cmd>lua require('package-info').hide()<cr>",
    {desc = "Hide package info"})
map("n", "<leader>nt", "<cmd>lua require('package-info').toggle()<cr>",
    {desc = "Toggle package info"})
map("n", "<leader>nu", "<cmd>lua require('package-info').update()<cr>",
    {desc = "Update package"})
